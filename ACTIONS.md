...
| Tham số | Ý nghĩa |
|---|---|
| `mode` | `""` chạy thường · `"--update"` ghi đè dữ liệu cũ · `"--dry-run"` chạy thử, không ghi Base |
...
- Mặc định: Page đã có thì **bỏ qua**, không đụng.
- `{"mode":"--update"}` → **ghi đè lại token** của mọi Page. Dùng khi đổi token Facebook.
...
| `mode` | — | `"--update"` = làm mới số liệu bài **đã có** (like/share/comment thay đổi theo thời gian) |
...
> Chống trùng theo `Post-ID`. Muốn số liệu luôn mới thì hẹn lịch chạy hằng ngày với `mode = --update`.
...
Đăng xong hệ thống ghi ngược lại dòng đó: `Trạng thái`, `Link bài đăng`, `Log`, và **hạ cờ cột `Đăng`** để automation không bắn lại.
...
Nên chạy với `--update` định kỳ để làm mới tổng chi tiêu.
...
node scripts/fetch-posts.js --update
